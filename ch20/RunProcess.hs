...
instance CommandLike (String -> IO String) where
    invoke func closefds input =
       return $ CommandResult (func input) (return (Exited ExitSuccess)) closefds
...
instance CommandLike ([String] -> IO [String]) where
    invoke func closefds input =
           return $ CommandResult linedfunc (return (Exited ExitSuccess)) closefds
